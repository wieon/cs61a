...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    for i in paragraphs:
        if select(i) == True:
            pick_string.append(i)
    if k <= len(pick_string)-1:
        return pick_string[k]
    else:
        return ''
    # END PROBLEM 1
...
def about(keywords: list[str]):
    ...
    def select(paragraph):
        words = set(split(lower(remove_punctuation(paragraph))))  # 去重
        for i in keywords:
            if i in words:
                return True
        else:
...
def accuracy(entered: str, source: str) -> float:
    ...
    right = 0
    wrong = 0
    max_words = max(len(entered_words), len(source_words))
    min_words = min(len(entered_words), len(source_words))
    if min_words == 0 and max_words == 0:  # Both empty
        return 100.0
    elif min_words == 0 and max_words != 0:  # Either one empty
        return 0.0
    else:  # Both not empty
        ...
        if len(entered_words) > len(source_words):
            ...
            wrong += diff
        return right / (right + wrong) * 100.0
...
def autocorrect(entered_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    #     if diff_function(entered_word, list_word, limit) < min_diff[0]:
    #         min_diff[0] = diff_function(entered_word, list_word, limit)
    #         min_diff[1] = list_word
    # if min_diff[0] > limit:
    #     return entered_word
    # else:
    #     return min_diff[1]
    ...
    if entered_word in word_list:
        return entered_word
    diff_list = [diff_function(entered_word, list_word, limit) for list_word in word_list]
    min_diff = min(zip(diff_list, word_list), key=lambda item:item[0])
    if min_diff[0] > limit:
        return entered_word
    else:
        return min_diff[1]
...
def furry_fixes(entered: str, source: str, limit: int) -> int:
    ...
    def select(a, b, count):
        ...
        if count > limit:
            return 0
        if a[0] != b[0]:
            return select(a[1:], b[1:], count+1) + 1
        else:
            return select(a[1:], b[1:], count)

    return select(entered, source, 0)
...
def minimum_mewtations(entered: str, source: str, limit: int) -> int:
    ...
    elif len(entered) == 0 or len(source) == 0: # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        return len(entered) + len(source)  # 妙！
    elif entered == source:
        return 0
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    for timestamps in timestamps_per_player:
        ...
        for i in range(0, len(timestamps)-1):
            li.append(timestamps[i+1] - timestamps[i])
        times.append(li)
    # END PROBLEM 9
