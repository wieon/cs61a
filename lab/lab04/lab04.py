...
def sum_tree(t):
    ...
    "*** YOUR CODE HERE ***"
    if is_tree(t):
        total = label(t) + sum([sum_tree(b) for b in branches(t)])
    else:
        total += label(t)
    return total
...
def only_paths(t, n):
    ...
    if any(new_branches):  # not empty
        return tree(label(t), [b for b in new_branches if b is not None])  # b is not 0
